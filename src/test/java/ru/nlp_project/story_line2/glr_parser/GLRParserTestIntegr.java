...
public class GLRParserTestIntegr {
  ...
  static class FactSerializer implements IFactListener {
    ...
    private String serializeFact(SentenceProcessingContext executionContext,
        Fact fact) {
  ...
  static class GrammarSerializationLogger implements IGLRLogger {
    ...
    public void detectedParseTree(SentenceProcessingContext context,
        ParseTreeNode tree, ParseTreeNode userTree, boolean validated,
        ParseTreeValidationException exception) {
      detectedParseTreeSerialized.add(
          serializeParseTree(context, tree, userTree, validated, exception));

    }

    private String serializeParseTree(SentenceProcessingContext context,
        ParseTreeNode tree, ParseTreeNode userTree, boolean validated,
        ParseTreeValidationException exception) {
      StringWriter sw = new StringWriter();
      PrintWriter pw = new PrintWriter(sw);
      if (validated)
        ...
        pw.print("<invalidated - " + userTree.toString() + "("
  ...
  public static void setUpClass() throws IOException {
    ...
    parserConfigDir = TestFixtureBuilder.unzipToTempDir(
        "ru/nlp_project/story_line2/glr_parser/GLRParserTestIntegr.zip");
    testable = GLRParser.newInstance(parserConfigDir + "/glr-config.json",
  ...
  public void testTest01() throws IOException {
    ...
    String result = grammarSerializationLogger.detectedParseTreeSerialized
    ...
    result = factSerializer.factsSerialized.stream().sorted()
        .collect(Collectors.joining("\n"));
    assertEquals(
  ...
  public void testTest01_10_000Iterations() throws IOException {
    for (int i = 0; i < 10_000; i++) {
      ...
      /*
       * result =
       * grammarSerializationLogger.detectedParseTreeSerialized.stream()
       * .sorted().collect(Collectors.joining("\n")); assertEquals(
       * "<validated - <S('Аравии'), 3,2 (24,17)>->[<Painter{PaintedMan.Who}('Аравии'), 3,2 (24,17)>->[<adj{PaintedMan.Color}('Саудовской'), 3,1 (24,10)>, <noun<rt, gram=\"femn\">('Аравии'), 4,1 (35,6)>]]>\n"
       * +
       * "<validated - <S('Аравия'), 4,2 (30,17)>->[<Painter{PaintedMan.Who}('Аравия'), 4,2 (30,17)>->[<adj{PaintedMan.Color}('Саудовская'), 4,1 (30,10)>, <noun<rt, gram=\"femn\">('Аравия'), 5,1 (41,6)>]]>\n"
