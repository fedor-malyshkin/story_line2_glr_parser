...
public class ParseTreeValidatorTest {
	...
	public void testAgreementSuccess_c() throws Exception {
		List<Token> tokens = tokenManager.splitIntoTokens("деревом топором бараном", true);

		String grammarText = "Root->S; S->Noun<c-agr=[1]> Noun<c-agr=[1] > Noun<c-agr=[1] >;";
		ParseTreeNode userRootSymbolNode =
				TestFixtureBuilder.createParseTree(grammarText, tokens, grammarManager);
	...
	public void testAgreementSuccess_MixAgreementGroups() throws Exception {
		List<Token> tokens = tokenManager.splitIntoTokens("дерево топор деревом топором", true);
	...
	/**
	 * ['розовый' (adj {adjf,gent,plur,qual}), 'розовый' (adj {accs,adjf,anim,plur,qual}), 'розовый'
	 * (adj {adjf,loct,plur,qual})] == ['розовый' (adj [{0=adj, 1=gent, 5=plur},]
	 * {adjf,gent,plur,qual}), 'розовый' (adj [{0=adj, 1=accs, 4=anim, 5=plur},]
	 * {accs,adjf,anim,plur,qual}), 'розовый' (adj [{0=adj, 1=loct, 5=plur},]
	 * {adjf,loct,plur,qual})] ['женщина' (noun {accs,anim,femn,noun,sing})] == ['женщина' (noun
	 * [{0=noun, 1=accs, 3=femn, 4=anim, 5=sing},] {accs,anim,femn,noun,sing})]
	 * 
	 * @throws Exception
	 */
	@Test
