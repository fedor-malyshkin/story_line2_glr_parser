...
public class NameFinderTest {
  ...
  public void setUp() {
    ...
    testable = glrParser.getNameFinder();
  }
  ...
  public void testPrestprocessTokens_2HyphenWords() {
    List<Token> tokens = tokenManager
        .splitIntoTokens("К нам пришел Коко-Дель-Рей и Иванов П.А..", false);
    testable.preprocessTokens(tokens);
  ...
  public void testDetectFIOKeywordEntrances() {
    ...
    testable.preprocessTokens(tokens);
    List<NameFinder.FIOEntry> fioKeywordEntrances =
        (List<NameFinder.FIOEntry>) testable.detectFIOKeywordEntrances(tokens);
   ...
   * {@link TokenManager.FIOKeywordToken}.
   * 
   * 
   * @throws IOException 
   */
  ...
  public void testDetectFIOKeywordEntrances_ForeignNamesSurnames() {
    List<Token> tokens = tokenManager.splitIntoTokens(
        "На встрече присутствовали: Беньямин Натаньяху, Катрин Денев, "
            + "Марчелло Мастрояни, Барак Обама, Пан Ги Мун, "
            + "Алексис Ципрас, Джон Керри, Мария Елена Боски, Микелла Витория Брамбилла,"
            + "Маттео Ренци и Сильвио Берлускони.",
        true);

    testable.preprocessTokens(tokens);
    Collection<NameFinder.FIOEntry> fioKeywordEntrances =
  ...
  public void testDetectFIOKeywordEntrances_UnknowSurnames() {
    ...
    assertEquals("наталье", fioKeywordEntrances.get(11).serialize());
    assertEquals("доброневскому виктору",
        fioKeywordEntrances.get(12).serialize());
  }
}
