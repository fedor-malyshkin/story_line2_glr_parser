...
public class TokenManagerImplTest {
	...
	public static void setUpClass() throws IOException {
		...
		glrParser = GLRParser.newInstance(parserConfigDir + "/glr-config.json", false);
	}
	...
	public void testModifyTokensByPlainKeywords() {
		keywordManagerMock = mock(IKeywordManager.class);
		testable.keywordManager = keywordManagerMock;

		when(keywordManagerMock.getKeywordSetsNameByIndex(eq(12))).thenReturn("keywordSetName");

		List<Token> tokens = testable.splitIntoTokens("Немного текста с кавычками - 'Госстрах', "
				+ "\"Энергогарант\", `МособлСтрой`, «Горгаз», «Два слова».", false);

		PlainKeywordEntrance entrance1 =
				new PlainKeywordEntrance(1, 3, 12, 12, 1, new Token(2, 1, "C", TokenTypes.WORD));
		PlainKeywordEntrance entrance2 =
				new PlainKeywordEntrance(22, 2, 12, 12, 0, new Token(4, 1, "E", TokenTypes.WORD));
		testable.modifyTokensByKeywords(tokens, Arrays.asList(entrance1, entrance2));
	...
	public void testDetectTokensVariant01() {
		...
		assertEquals("0-1=\", 1-8=Немножко, 10-6=текста, 17-2=на, 20-7=русском, 28-5=языке, "
				+ "33-1=\", 34-1=,, 36-6=сказал, 43-6=Кузбма, 50-7=Прутков, 57-1=!", fact);
	}

	@Test
	public void testDetectTokensVariant02() {
		List<Token> tokens = testable.splitIntoTokens("Обычный текст - с точкой.", false);
		String fact = serializeTokens(tokens);
		assertEquals("0-7=Обычный, 8-5=текст, 14-1=-, 16-1=с, 18-6=точкой, 24-1=.", fact);
	}

	private String serializeTokens(List<Token> tokens) {
		return tokens.stream()
				.map((t) -> String.format("%d-%d=%s", t.getFrom(), t.getLength(), t.getValue()))
	...
	public void checkRegistryWithNumbers() {
		List<Token> tokens = testable.splitIntoTokens("Немного - 17 тарелок.", false);
	...
	public void checkLat() {
		List<Token> tokens = testable.splitIntoTokens("Немного текста in English.", false);
		// 1 текста
