...
public class Interpreter {
  ...
  public class Fact {
    private Map<String, FactField> fieldsMap =
        new TreeMap<String, Interpreter.FactField>();
    ...
    public void setFieldValue(String field, String value, int from,
        int length) {
      FactField factField = fieldsMap.get(field);
      if (null == factField)
        throw new IllegalStateException("not existing field name:" + field);
  ...
  public class FactManager {
     ...
     */
    Map<String, AtomicInteger> occCounter =
      ...
      for (TreeInterpData treeInterpData : treeInterpDatas) {
        ...
        if (null == fact)
          throw new IllegalStateException(
              "Incorrect fact '" + treeInterpData.fact + "'");
    ...
    public void generateFacts() {
      for (TreeInterpData treeInterpData : treeInterpDatas) {
        Fact f = getFactWithEmptyField(treeInterpData.fact,
            treeInterpData.field, treeInterpData.from, treeInterpData.length);

        if (treeInterpData.interpData.getFromFactName() != null)
          // { ZZZZ.ZZZZZ from YYY.YYY }
          f.setFieldValue(treeInterpData.field,
              previousFactFieldsMap
                  .get(treeInterpData.interpData.getFromFactName() + "."
          ...
          // { YYY.YYYY = 'value'}
          f.setFieldValue(treeInterpData.field,
              treeInterpData.interpData.getValue(), treeInterpData.from,
              treeInterpData.length);
        else
          // { YYYY.YYYY }
          // use #setFieldValue in mandatory case - reimplemented in
          // FactListFacade
          f.setFieldValue(treeInterpData.field,
              generateFactValue(treeInterpData), treeInterpData.from,
              treeInterpData.length);
      }
    ...
    public List<Fact> getFacts() {
      List<Fact> result = new LinkedList<Interpreter.Fact>();
    ...
    /**
     * Get fact with empty field name from {@link #innerFactsMap}, append to
     * list in neccessary case.
     * 
     * In addition to it check all previous _intersectioned_ facts and get them
     * to value replacement (can intersect only fact wich was added previusly
     * with the same field).
     * 
     * @param fact
     * @param field
     ...
     * @return
     */
    private Fact getFactWithEmptyField(String fact, String field, int from,
        int length) {
      Fact result = null;
    ...
    private void storeToMapPreviousFacts() {
      ...
      for (int i = previousFacts.size() - 1; i >= 0; i--) {
        ...
        for (FactField factField : fact.fieldsMap.values())
          previousFactFieldsMap.put(fact.name + "." + factField.name,
              factField.value);
      }
    }
  ...
  public static Interpreter newInstance(ConfigurationReader configurationReader,
      TokenManager tokenManager) throws IOException {
  ...
  TreeInterpData newTreeInterpData(int from, int length, String fact,
      String field, SymbolInterpData interpData, ParseTreeNode parseTreeNode) {
    return new TreeInterpData(from, length, fact, field, interpData,
        parseTreeNode);
  }
  ...
  public void processEntrances(SentenceProcessingContext context,
    ...
    for (GrammarKeywordEntrance grammarKeywordEntrance : grammarEntrances) {
      ...
      try {
        treeInterpDatas =
            readEntranceTree(grammarKeywordEntrance.getParseTreeNode());
      } catch (Exception e) {
      ...
      grammarKeywordEntrance
          .setInterpretationResult(new InterpretationResult(facts));
  ...
  protected List<Fact>
      processTreeInterpDatas(List<TreeInterpData> treeInterpDatas) {
   ...
   * @return
   */
  protected List<Fact> processTreeInterpDatas(
      List<TreeInterpData> treeInterpDatas,
      InterpretationResult interpretationResult) {
    FactManager factManager =
        new FactManager(treeInterpDatas, interpretationResult);
    factManager.initialize();
    factManager.generateFacts();
    return factManager.getFacts();
  }

  List<TreeInterpData> readEntranceTree(ParseTreeNode parseTreeNode)
      throws Exception {
    List<TreeInterpData> result = new LinkedList<Interpreter.TreeInterpData>();
    parseTreeNode.walkPostOrderLeafFirst(
        new ParseTreeNode.IPostOrderWalkLeafFirstProcessor() {
          @Override
            ...
            if (node.symbol != null
              ...
              if (s.getInterpDatas() != null)
                for (SymbolInterpData id : s.getInterpDatas())
                  result.add(new TreeInterpData(node.tokenFrom,
                      node.tokenLength, id, node));
            }
          }
        });
    return result;
  ...
  protected void readFactFile(InputStream inputStream) throws IOException {
    ...
    for (Map<String, Object> entry : entries) {
      ...
      if (name == null)
        throw new IllegalStateException(
            "Fact with no name: " + entry.toString());
