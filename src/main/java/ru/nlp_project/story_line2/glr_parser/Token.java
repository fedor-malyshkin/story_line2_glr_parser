...
/**
 * 
 * Объект-токен, содержащий в себе всю необходимую информацию по каждому токену внутри предложения:
 * тип, значение, длинна и т.д....
 * 
 * WARNING: объект после создания не должен меняться, как и вся его грамматическая часть, т.к. они
 * многократно анализируются и проверяются в других вариантах разбора дерева. В случае необходимости
...
public class Token {
	...
	public class Lexeme {
		...
		public Lexeme(String id, String lemm, String lexemeValue, Grammemes grammemes,
				boolean exactMatch) {
			super();
			this.id = id;
			this.lemm = lemm;
		...
		public Lexeme clone() {
			return new Lexeme(this.id, this.lemm, this.lexemeValue, this.grammemes.clone(),
					this.exactMatch);
		}
	...
	boolean hReg1 = false;
	/**
	 * В верхнем регистре стоит первая буква слова и как минимум еще одна буква слова, как например,
	 * в слове «МосСтрой».
	 */
	boolean hReg2 = false;
	/**
	 * Само значение {@link IKeywordEntrance} (в случае {@link GrammarKeywordEntrance} через него
	 * можно получить доступ к
	 * {@link ru.nlp_project.story_line2.glr_parser.InterpreterImpl.InterpretationResult} для
	 * интерпретации).
	 */
	...
	/**
	 * Слово или группа слов c открывающей кавычкой перед первым символом и без закрывающей кавычки
	...
	/**
	 * Слово или группа слов c закрывающей кавычкой после последнего символа и без закрывающей
	 * кавычки перед первым символом.
	 */
	...
	public void addLexeme(String id, String lemm, String lexemeValue, Grammemes grammemes,
			boolean exactMatch) {
	 ...
	 * 
	 * При необходимости осуществлять манипуляции со списком - {@link Token#getLexemesIterator()}.
	 * 
	 * @return
	 */
	 ...
	 * 
	 * ВНИМАНИЕ: возвращется копия списка с настоящими узлами - поэтому изменения в список
	 * фактически не вносятся. А вот манипуляции с объектами списка отражаются на реальном
	 * содержании объекта.
	 * 
	 * При необходимости осуществлять манипуляции со списком - {@link Token#getLexemesIterator()}.
	 * 
	...
	public LinkedList<Lexeme> getLexemesListCopy(boolean includePredicted) {
		if (includePredicted)
			return lexemes.stream().collect(Collectors.toCollection(LinkedList::new));
		else
	...
	private String serializePOS() {
		...
		for (int i = 0; i < lexemes.size(); i++)
			sts.add(lexemes.get(i).lemm + "(" + lexemes.get(i).grammemes.getPOS().toString() + ")");
	...
	public String toString() {
		if (kwName != null)
			...
			return String.format("('%s', %d,%d, @%s, #%s)", value, from, length, type.toString(),
					serializePOS());
	}
