...
/**
 * Класс проверки помет-ограничений на токены (лексемы в их составе) и
 * соотвествующие им символы.
 * <p/>
 * Код предназначен для проверки соотвествия условиям как для токенов (сигнатура
 * "matchXXXX(SymbolExtData, SymbolExt, Token)", вызывается из реализации
 * {@link IGLRTokenMatcher}) так и для нетерминалов (сигнатура
 * "matchXXXX(SymbolExtData, SymbolExt, ParseNodeTree)", вызывается из
 * {@link ParseTreeValidator#validateTree(SentenceProcessingContext, ParseTreeNode)}
 * )
 * <p/>
 * ВНИМАНИЕ: в данном классе по максимуму сконцентриррованы все проверки помет и
 * помет-ограничений для минимизации их разбредания по коду парсера.
 * 
 * @author fedor
 *
 */
...
public class SymbolRestrictionChecker {
  ...
  public boolean match(SymbolExt symbolExt, Token token, ParseTreeNode node) {
    ...
    for (Entry<SymbolExtDataTypes, SymbolExtData> entry : entrySet) {
      if (token != null && !entry.getKey().isApplyToTerminal())
        ...
        throw new IllegalStateException(
            "Restiriction is not applicable to non-terminal: "
                + entry.getKey());
  ...
  public boolean match(SymbolExtDataTypes symbolExtDataType,
      SymbolExtData symbolExtData, SymbolExt symbolExt, Token token,
      ParseTreeNode node) {
    ...
    case gram_ex:
      return matchGramEx(symbolExtData.getGrammValue(), symbolExt, token, node);
    ...
    default:
      throw new IllegalStateException(
          "Unknown restriction: " + symbolExtDataType);
    }
  }
  ...
  boolean matchDict(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token) {
    // TODO Auto-generated method stub
    return false;
  ...
  private boolean matchDict(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchFW(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchFW(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchGram(Grammemes grammValue, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchGram(Grammemes grammValue, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchGramEx(Grammemes grammValue, SymbolExt symbolExt,
      ParseTreeNode node) {
    return matchGramEx(grammValue, symbolExt, node.token);
  }

  boolean matchGramEx(Grammemes grammemes, SymbolExt symbolExt, Token token) {
    for (Lexeme l : token.getLexemesListCopy()) {
      if (l.getGrammemes().hasAllEx(grammemes))
        return true;
    }
    return false;
  }

  private boolean matchGramEx(Grammemes grammValue, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
      return matchGramEx(grammValue, symbolExt, token);
    else
      return matchGramEx(grammValue, symbolExt, node);

  }

  boolean matchGU(List<GUBlock> guValue, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  /**
   * 
   * Помета GU (grammar union) предоставляет более широкие возможности 
   * использования граммем в грамматиках. В своей самой простой форме GU=["nom,pl"] 
   * эта помета аналогична помете gram: она проверяет грамматические характеристики 
   * (в примере выше — «именительный падеж множественного числа») у каждого омонима 
   * и если находится омоним, удовлетворяющий этому условию, то правило срабатывает.
  
  Граммемы записываются через запятую в квадратных скобках [ ]. Отрицание 
  отдельных граммем в этой записи запрещено. Отрицание ~ перед квадратными 
  скобками означает, что пересечение перечисленных граммем с граммемами каждого 
  омонима пусто, т.е. если интерпретация хотя бы одного омонима подойдет по 
  множество граммем в квадратных скобках, то правило с таким ограничением не 
  сработает.
  
  Амперсанд & перед квадратными скобками означает, что парсер будет рассматривать 
  граммемы не у каждого омонима по отдельности, а одновременно у объединения 
  грамматических признаков всех омонимов.
  
  Кроме того помета GU позволяет записывать дизъюнкцию нескольких таких условий. 
  Через вертикальную черту | («или») можно записать несколько разных списков 
  граммем и правило сработает в том случае, если слово удовлетворяет хотя бы 
  ...
  boolean matchGU(List<GUBlock> guList, SymbolExt symbolExt, Token token) {
    ...
    while (guIter.hasNext()) {
      ...
      // вариант когда все лексемы должны подходить под условия
      {
        ...
        for (Lexeme l : token.getLexemesListCopy())
          set.addAll(l.getGrammemes().getMainGrammems());
  ...
  private boolean matchGU(List<GUBlock> guValue, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchHReg1(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchHReg1(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchHReg2(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchHReg2(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchKWSet(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchKWSet(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchKWSetF(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    LinkedList<ParseTreeNode> terminalNodes = collectTerminalNodes(node);
  ...
  private boolean matchKWSetF(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchKWSetL(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    LinkedList<ParseTreeNode> terminalNodes = collectTerminalNodes(node);
    return matchKWSet(symbolExtData, symbolExt, terminalNodes.getFirst());
  }
  ...
  boolean matchKWType(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token) {
    return hierarchyManager.isParent(symbolExtData.getKwTypeValue(),
        token.getKwName());
  }
  ...
  boolean matchKWTypeF(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    LinkedList<ParseTreeNode> terminalNodes = collectTerminalNodes(node);
    return matchKWType(symbolExtData, symbolExt,
        terminalNodes.getFirst().token);
  }
  ...
  private boolean matchKWTypeF(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchKWTypeL(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    LinkedList<ParseTreeNode> terminalNodes = collectTerminalNodes(node);
  ...
  private boolean matchKWTypeL(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  private boolean matchLat(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    return matchLat(symbolExtData, symbolExt, node.token);
  }

  boolean matchLat(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token) {
    return token.isLat();
  }
  ...
  boolean matchLQuoted(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchLQuoted(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchLReg(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchLReg(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchMW(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token) {
    if (token.getClass() == PlainKeywordToken.class
  ...
  private boolean matchMW(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchNoHom(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchNoHom(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchQuoted(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchQuoted(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchRQuoted(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchRQuoted(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchRx(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
  ...
  private boolean matchRx(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchRxF(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    LinkedList<ParseTreeNode> terminalNodes = collectTerminalNodes(node);
  ...
  private boolean matchRxF(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  boolean matchRxL(SymbolExtData symbolExtData, SymbolExt symbolExt,
      ParseTreeNode node) {
    LinkedList<ParseTreeNode> terminalNodes = collectTerminalNodes(node);
  ...
  private boolean matchRxL(SymbolExtData symbolExtData, SymbolExt symbolExt,
      Token token, ParseTreeNode node) {
    if (token != null)
  ...
  public void removeUnmatchingLexemesGram(Grammemes grammemes,
      SymbolExt symbolExt, Token token) {
    Iterator<Lexeme> iterator = token.getLexemesIterator();
    while (iterator.hasNext()) {
      Lexeme l = iterator.next();
      if (!l.grammemes.hasAll(grammemes.getMainGrammems()))
        iterator.remove();
    }
  }
   ...
   * @param grammemes
   */
  public static void removeUnmatchingLexemesGram(Token token,
      Set<GrammemeEnum> grammemes) {
    Iterator<Lexeme> iterator = token.getLexemesIterator();
    while (iterator.hasNext()) {
      Lexeme l = iterator.next();
  ...
  public void removeUnmatchingLexemesGramEx(Grammemes grammemes,
      SymbolExt symbolExt, Token token) {
    Iterator<Lexeme> iterator = token.getLexemesIterator();
    while (iterator.hasNext()) {
      Lexeme l = iterator.next();
      if (!l.grammemes.hasAllEx(grammemes))
        iterator.remove();
    }

  }

  public void removeUnmatchingLexemesGU(List<GUBlock> guList,
      SymbolExt symbolExt, Token token) {
   ...
   * @return
   */
  public static int matchScore(Grammemes grammemes1, Grammemes grammemes2,
    ...
    if (agreement != null) {
      ...
      case gc_agr:
        ...
        break;
      default:
        throw new IllegalStateException("Unknown restriction: " + agreement);
      }
    }
    int result = 0;
    for (int i = 0; i < 14; i++) {
      if (grammemes1.matchBy(grammemes2, i))
        result += scores[i];
      else
        result -= scores[i];
    }
