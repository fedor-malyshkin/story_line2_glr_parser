...
public class PlainKeywordEntranceDetector {
  class AnalyseThread {
    ...
    int currTokenPos = 0;
    /**
     * Предыдущий номер обнаруженного многословного слова из набора (используеся для многсловных ключевых слов)
     */
    ...
    public String toString() {
      return "AnalyseThread [currTokenPos=" + currTokenPos + ", keywordPos="
          + keywordPos + ", multiword=" + multiword + ", prevMultiwordPart="
          + prevMultiwordPart + ", tokens=" + atTokens + ", processed="
          + processed + "]";
    }
  ...
  public PlainKeywordEntranceDetector(PlainKeywordTrie trie,
      List<Token> tokens) {
    this.reader = new PlainKeywordTrieReader(trie);
    this.tokens = tokens;
    this.results = new ArrayList<PlainKeywordEntrance>();
    this.analyseThreads =
        new ArrayDeque<PlainKeywordEntranceDetector.AnalyseThread>();
  }
   ...
   * Принцип работы:
   * <ul>
   * <li>Из стека извлекается текущий результат поиска и копируется для всех омонимов, базовых форм слова (лемм) и оригинальных форм 
   * (кроме того им передается пустой результат анализа для возможного начала нового вхождения);</li>
   * <li>Каждая копия проверяется на вхождение в trie - в случае вхождения проверяется результат данных (особая проверка для 
   * многословных ключевых слов);</li>
   * <li>В случае вхождения результат анализа изменяется и помещается в стек;</li>
   * <li>В случае не_вхождения - ничего не делается и как результат вариант анализа выкидывается из процесса поиска;</li>
  ...
  /**
   * Создать копии анализа для всех базовых форм омонимов (лемм) и оригинальных форм токена и выполнить поиск.
   * 
   * @param originalTokens
   * @param tokens2
   * @param analyseThread
  ...
  private void cloneSearchFor(AnalyseThread analyseThread) {
    ...
    for (Lexeme lexem : token.getLexemesListCopy()) {
      List<PlainKeywordTrieBuilder.KeywordInfo> list =
          reader.analyse(lexem.getLemm());
      if (list.size() > 0 && !analyseThread.processed)
        analyseSearchResults(list, analyseThread, true);
    }
    List<PlainKeywordTrieBuilder.KeywordInfo> list =
        reader.analyse(token.getValue());
    if (list.size() > 0 && !analyseThread.processed)
  ...
  /**
   * Выполнить анализ вхождения в trie для всех базовых форм омонимов (лемм) и оригинальных форм токена и выполнить поиск.
   * 
   * @param originalTokens
   * @param i
   */
   ...
   * 
   * Анализируем все полученные результатаы и...
   * в случае однословного ключа - просто добавляем результат вхождения;
   * в случае многословного ключа - 
   ...
   */
  private void analyseSearchResults(
      List<PlainKeywordTrieBuilder.KeywordInfo> analyseResults,
    ...
    for (KeywordInfo kwi : analyseResults) {
      ...
      if (kwi.multiword) {
        ...
        if (kwi.multiwordPart == 0) {
          ...
          AnalyseThread newAnalyseThread = new AnalyseThread(
              aT.currTokenPos + 1, true, kwi.keywordPos, 0, newAtTokens);
        ...
        if (aT.multiword && aT.keywordPos == kwi.keywordPos
          ...
          if (kwi.hasContinue) {
            ...
            results.add(new PlainKeywordEntrance(
                ...
                newAtTokens.get(kwi.options.mainWord)));
        ...
        results.add(new PlainKeywordEntrance(aT.currTokenPos, 1, kwi.keywordSet,
            kwi.keywordPos, 0, aT.atTokens.get(kwi.options.mainWord)));
        // больше не обрабатываем данный вариант анализа
        aT.processed = true;
      }
    }
  ...
  private boolean chechRestrictions(KeywordInfo kwi,
    ...
    if (options.grammTree != null)
      for (Entry<Integer, Set<GrammemeEnum>> entry : options.grammTree
          .entrySet()) {
        Token token = analyseThread.atTokens.get(entry.getKey());
        if (!SymbolRestrictionChecker.matchGram(token, entry.getValue()))
          return false;
        SymbolRestrictionChecker.removeUnmatchingLexemesGram(token,
            entry.getValue());
        if (!token.hasLexemes())
          return false;
      }
  ...
  /**
   * 
   * Выполнить проверку соответствия данных имеющимся опциям (для одного независимого слова в отдельности, т.е. часть проверок, 
   * допустимых лишь для многословных ключевых слов -- пропускаем).
   * 
   * 
   * @param kwi
   * @param analyseThread
   ...
   * @return
   */
  private boolean validateSingleWordSearchResult(KeywordInfo kwi,
      AnalyseThread analyseThread, boolean lemm) {
    LookupOptions options = kwi.options;
