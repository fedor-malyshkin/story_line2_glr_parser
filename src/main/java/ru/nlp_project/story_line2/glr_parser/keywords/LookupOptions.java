...
class LookupOptions {
	/**
	 * Помета описывает согласование между двумя словами в ключе. Возможны два типа согласования: по
	 * роду, числу и падежу (agr=gnc_agr); по падежу (agr=c_agr).
	 */
	...
	/**
	 * Значения этого поля — лемма, которая заменяет обнаруженное слово или группу слов. Например,
	 * "-lemm="Россия"" заменяет лемму группы "российская федерация" на единственную лемму -
	 * "Россия".
	 */
	@Parameter(names = "-lemm")
	String lemm;
	/**
	 * Значения этого поля — граммемы, которые применяются к ключу. Например, "-gram="sg"" означает,
	 * что статье соответствуют только формы ключа в единственном числе.
	 */
	@Parameter(names = "-gramm", converter = OptionGrammConverter.class)
	Set<GrammemeEnum> gramm;
	/**
	 * R какому из слов многословного ключа применяется помета gram. "-gramm-1="re,rewrwer,dsada""
	 */
	...
	void convertGrammTree() {
		...
		for (Map.Entry<String, String> entr : grammTreeRaw.entrySet()) {
			...
			grammTree.put(Integer.parseInt(entr.getKey()), EnumSet.copyOf(gramm.getGrammemes()));
		}
	}
}
