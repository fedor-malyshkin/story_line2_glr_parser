...
/**
 * Основной класс для работы с ключевыми словами (чтение слов из файлов, 
 * составление внутренних структур, поиск в предложениях, составление 
 * оптимального распределения по предложению (с условием максимального покрытия)).

 * Общий алгоритм работы такой:
 * <p>
Упрощенно: для каждого набора ключей (keyword set) создается дерево (trie) потом по ним осуществляется поиск слов на предмет вхождения.
</p>
...
<p>
Фактически для разрешения этого каждое слово набора ключей (как и сам набор) нумеруется, а в случае многословного ключа (multiword key) 
нумеруется и слово в ключе, указанная информация (среди прочего) вносится для каждого слова/слова ключа в 
соответствующий объект, прикрепленный к внешним узлам дерева. Указанные данные используются для определения 
вхождения или границ вхождения. Кроме этого для определения всех вхождений, включая перекрывающиеся, поиск идет по 
всем направлениям для всех слов, что реализуется с использованием специального объекта ({@link CoverageVariant}), сохраняющего в 
себе статус поиска, который в случае успеха создает записи о вхождении (многословных) ключей в текст.
</p>
Порядок построения такой:
<ul>
<li>Для однословных ключей в дерево просто вносится слово с соответствующим объектом данных;</li>
<li>Для многословных ключей в дерево вносится каждое слово с соответствующим объектом данных (с указанием на 
вхождение в набор ключей и номер слова);</li>
</ul>
...
<li>Для каждого омонима, леммы и исходной формы слова копируется статус текущего поиска в свой отдельный поток 
поиска с целью проработки всех возможных вариантов</li>
</ul>

Порядок использования такой:
<ul>
<li>Добавляются необходимые ключевые слова ({@link #addKeywordSet(String, List, Map, String)})</li>
<li>Ищются все возможные вхождения ({@link #detectKeywordEntrances(String, List)})</li>
<li>Вычисляется оптимальное по покрытию непересекающееся множество ключевых слов({@link #calculateOptimalKeywordsCoverage(List, int)})</li>
</ul>
 * 
 * <br/>
 * MULTITHREAD_SAFE: TRUE (все имеющиеся члены класса меняются лишь при 
 * инициализации, а помле осуществляется доступ лишь на чтение)
 * 
 * @author fedor
 *
 */
public class KeywordManager {
  ...
  private class CoverageVariant {
    ...
    /**
     * Создать вариант на основании списка вхождений (отсортированы от конечных вхождений к основным)
     * 
    * @param list
    * @return
    */
  ...
  public static class OptionAgrConverter
      implements IStringConverter<SymbolExtDataTypes> {
  ...
  public static class OptionGrammConverter
      implements IStringConverter<Set<GrammemeEnum>> {
  ...
  protected void addKeywordEntry(PlainKeywordTrieBuilder trieBuilder,
      String kwEntry, LookupOptions globalOptions, int entryPos) {
    ...
    if (strings.size() == 1)
      ...
      for (String string : strings) {
        trieBuilder.addWord(string, trieBuilder.createMultiwordKewordInfo(
            entryPos, counter < strings.size() - 1, counter, localOptions));
        counter++;
      }
    }
  }
  ...
  /**
   * Добавить набор ключевых слов (совместно с опциями) из {@link ru.nlp_project.story_line2.glr_parser.DictionaryManager}.
   * 
   * 
  ...
  public void addKeywordSet(String keywordSetName, List<String> keywords,
    ...
    if (keywordSets.contains(keywordSetName))
      throw new IllegalStateException("Keyword set '" + keywordSetName
          + "' already added to keyword base.");
    keywordSetToArgsMap.put(keywordSetName, entry);
    PlainKeywordTrieBuilder trieBuilder =
        createPlainKeywordTrieBuilder(keywordSetName);
  ...
  /**
   * Рассчитать оптимальное покрытие токенов (предложение) ключевыми словами (с целью максимизации покрытия).
   * 
   * <ul>
   * <li>На вход подается отсортированный в порядке убывания список вхождений ключевых слов в предложение и кол-во токенов в предложении.</li>
   * <li>Проход начинается с хвоста предложения к началу, при этом формируются все возможные варианты покрытия.</li>
   * <li>Вариант покрытия - это возможная комибинация всех имеющихся отрезков, построенная таким образом, что они не перекрываются.</li>
   * <li>По завершении выполняется поиск максимального и он выбирается в качестве оптимального.</li>
   * <li>На текущий момент промежуточное удаление заранее не выигрышных вариантов покрытия не делается (хотя вся эта информация есть), 
   * т.к. их вычисление достаточно трудоемкая процедура и вполне возможно просто обменять время вычисления на излишнюю память на 
   * период вычисления.</li>
   * </ul>  
   * 
   * WARNING: очень дорогой алгоритм по по треблению к памяти - для 21 кол-во 
   *временных объектов может равняться 2 млн.!!!
   * 
   * @param kwes
   * @param tokensLength
   * @return
  ...
  public List<? extends IKeywordEntrance> calculateOptimalKeywordsCoverage(
    ...
    // dumb one
    coverageVariants.add(new CoverageVariant(0, tokensLength,
        Collections.<IKeywordEntrance> emptyList()));
    ...
    if (!coverageVariants.isEmpty()) {
      ...
      int maxCovers = coverageVariants.stream().mapToInt(c -> c.coverageLength)
          .max().orElse(0);

      Comparator<CoverageVariant> minElComp =
          new Comparator<CoverageVariant>() {
            @Override
      ...
      if (coverageVariants.size() > 1_000) {
        optional = coverageVariants.parallelStream()
            .filter(c -> c.coverageLength == maxCovers).sorted(minElComp)
            .findFirst();
      } else
        optional =
            coverageVariants.stream().filter(c -> c.coverageLength == maxCovers)
       ...
       * 
       * @Override public int compare(CoverageVariant o1, CoverageVariant o2) {
       * return o1.coverageLength - o2.coverageLength; } });
       */
  ...
  protected void combineLookupOptions(LookupOptions localOptions,
      LookupOptions globalOptions) {
   ...
   * Проходим по всем уже имеющимся и:
   * <ul>
   * <li>Создаем новый (на базе данных имеющегося), если окончание текущего вхождения меньше/равно начала самого раннего токена в покрытии.</li>
   * <li>Создаем новый (на базе данных имеющегося), если окончание текущего вхождения больше начала самого раннего токена в покрытии 
   * (при этом из нового варианта удаляются вхождения, создающие подобное условие).</li>
   * </ul>
  ...
  private void createCoverageVariants(List<CoverageVariant> coverageVariants,
    ...
    for (int i = 0; i < origSize; i++) {
      ...
      if (kweEnd <= dOpt.from) {
         ...
         */
        ArrayList<IKeywordEntrance> list =
            new ArrayList<IKeywordEntrance>(dOpt.contains);
        // добавляем наше вхождение
        list.add(kwe);
        coverageVariants.add(new CoverageVariant(
            dOpt.coverageLength + kwe.getLength(), kwe.getFrom(), list));
      } else {
        /*
         * Создаем новый (на базе имеющегося), если окончание текущего вхождения
         * больше начала самого раннего токена в покрытии (при этом из нового
         * варианта удаляются вхождения, создающие подобное условие).
         */
        LinkedList<IKeywordEntrance> list =
            new LinkedList<IKeywordEntrance>(dOpt.contains);
  ...
  private PlainKeywordTrieBuilder
      createPlainKeywordTrieBuilder(String keywordSetName) {
    PlainKeywordTrieBuilder trieBuilder =
  ...
  public List<PlainKeywordEntrance>
      detectPlainKeywordEntrances(String keywordSetName, List<Token> tokens) {
    if (keywordSetName.equalsIgnoreCase(KWS_NAME_FIO))
      return Collections.emptyList();
    PlainKeywordTrie keywordTrie = keywordSetToTrieMap.get(keywordSetName);
    if (keywordTrie == null)
      throw new IllegalStateException("Unknown keywordSet: " + keywordSetName
          + ". Have: " + keywordSetToTrieMap.keySet());
    // main word
   ...
   */
  protected LookupOptions parseEntryString(String kwEntry,
      List<String> strings) {
    String toSpit = kwEntry;
    int sepPos = StringUtils.indexOf(kwEntry, "|");
    if (sepPos != -1)
      toSpit = kwEntry.substring(0, sepPos);
    List<String> strs = TokenManager.splitIntoStrings(toSpit);
    combineExlamationSymbol(strs);
    for (int i = 0; i < strs.size(); i++)
      strings.add(strs.get(i));
    if (sepPos != -1)
      return parseLookupOptions(
          kwEntry.substring(sepPos + 1, kwEntry.length()));
   ...
   * 
   * Опции это значения в формате "-key=value или -key value". 
   * При наличии пробелов в значении - должно включаться в <b>двойные кавычки</b>.
   * 
   * 
   * @param optionsRaw
   * @return
  ...
  LookupOptions parseLookupOptions(String optionsRaw) {
    ...
    optionsRaw = StringUtils.replacePattern(optionsRaw, "-gramm-([0-9]) *",
        "-gramm-$1=");
  ...
  private void
      sortKeywordEntranceByStartReverse(List<? extends IKeywordEntrance> kwes) {
  ...
  private void
      sortKeywordEntranceByStart(List<? extends IKeywordEntrance> kwes) {
    Collections.sort(kwes, new Comparator<IKeywordEntrance>() {
      @Override
      public int compare(IKeywordEntrance o1, IKeywordEntrance o2) {
  ...
  public static KeywordManager
      newInstance(ConfigurationReader configurationReader) {
   ...
   * 
   * 1) Сортировка по уменьшению площади покрытия (DESC).
   * 2) Удаление сегментов, которые полностью входят в большие.
   * 3) ПОвторять до тех пор пока не получится ничего удалять.
   * 
   * @param entrances
   */
  public void
      simpleCoverageFiltering(List<? extends IKeywordEntrance> entrances) {
    ...
    for (int i = 0; i < entrances.size() - 1; i++) {
      ...
      List<? extends IKeywordEntrance> subList =
          entrances.subList(i + 1, entrances.size());
