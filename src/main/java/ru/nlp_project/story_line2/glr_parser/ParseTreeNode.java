...
public class ParseTreeNode {
   ...
   * списком - {@link ParseTreeNode#getLexemesIterator()}.
   * 
   * @return
   */
  ...
  public String toString() {
    ...
    if (token != null)
      sb.append(String.format("<%s('%s'), %d,%d (%d,%d)>", symbol,
          token.getValue(), tokenFrom, tokenLength, from, length));
    else
      sb.append(String.format("<%s(), %d,%d (%d,%d)>", symbol, tokenFrom,
          tokenLength, from, length));
  ...
  /**
   * 
   * Walk tree in level-order and visit each node on same level (left-right, top-down) (non-recursive method).
   * Проход через дерево сверху-вниз по уровням (пересекающим все ветви дерева).
   * 
  ...
  public void walkInOrder(boolean enableCut, IInOrderWalkProcessor processor) {
    ...
    Deque<ParseTreeNodeVisitData> parentStack =
        new LinkedList<ParseTreeNodeVisitData>();
    ...
    while (!parentStack.isEmpty()) {
      ...
      while (iterator.hasPrevious()) {
        ...
        if (!enableCut || !hasCutRestriction(node))
          parentStack
              .addFirst(new ParseTreeNodeVisitData(node, node.isTerminal));
      }
    }
  }
   ...
   * 
   * Walk tree in level-order and visit each node on same level (left-right, top-down) (non-recursive method).
   * Проход через дерево сверху-вниз по уровням (не пересекающим одну ветвь дерева).
   * 
   *  See: <a href="https://en.wikipedia.org/wiki/Tree_traversal">wiki page</a>
   * @throws Exception 
   * 
  ...
  public void walkLevelOrder(ILevelOrderWalkProcessor processor) {
    ...
    int level = 0;
    ParseTreeNode currNode = this;
    Deque<ParseTreeNodeVisitData> nextNodes =
        new LinkedList<ParseTreeNodeVisitData>();
   ...
   * При обходе в первую очередь обрабатываются конечные узлы (листья), потом их родители.
   * Т.е. при посещении любого узла, ты уже можешь быть уверен, что в дочерних узлах уже был.
   * 
   * See: <a href="https://en.wikipedia.org/wiki/Tree_traversal">wiki page</a>
   * @throws Exception 
   * 
  ...
  public void walkPostOrderLeafFirst(IPostOrderWalkLeafFirstProcessor processor)
    ...
    Deque<ParseTreeNodeVisitData> visitStack =
        new LinkedList<ParseTreeNodeVisitData>();
